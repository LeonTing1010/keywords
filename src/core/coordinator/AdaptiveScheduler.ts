...
import { FailureType, RecoveryManager, RecoveryStrategy } from './RecoveryManager';
...
export class AdaptiveScheduler {
  ...
  public registerWorkflow(config: WorkflowConfig): void {
    ...
    logger.info(`Workflow registered: ${config.workflowId}`, { 
  ...
  private async scheduleSteps(
    ...
    const stepPromises = stepsToExecute.map(async step => {
      ...
      try {
        logger.info(`Executing step: ${step.stepId}`, { agentId: step.agentId });
        ...
        if (!this.recoveryManager.canProceed(step.agentId)) {
          logger.warn(`Skipping step due to recent failures: ${step.stepId}`);
          step.status = 'skipped';
        ...
        logger.info(`Step completed: ${step.stepId}`);
      } catch (error: any) {
        // 记录失败
        step.status = 'failed';
        step.endTime = Date.now();
        ...
        logger.warn(`Step failed: ${step.stepId}`, { 
          error, 
          recovery: recoveryAction.strategy 
        });
        ...
        switch (recoveryAction.strategy) {
          case RecoveryStrategy.RETRY:
            // 重置步骤状态，等待重试
            step.status = 'pending';
            logger.info(`Scheduling retry for step: ${step.stepId}`, { 
          ...
          case RecoveryStrategy.SKIP:
            // 标记为已跳过
            step.status = 'skipped';
            logger.info(`Skipping failed step: ${step.stepId}`);
            break;
