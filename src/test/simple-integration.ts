...
async function runSimpleIntegrationTest() {
  ...
  try {
    ...
    // 直接测试Agent
    console.log('\n测试ProblemMiner:');
    const agentResult = await problemMiner.execute({
      data: { keyword: '远程办公' },
      context
    });
    ...
    if (agentResult.status === 'success') {
      ...
      console.log(`发现问题数量: ${data.problems.length}`);
      console.log('\n问题示例:');
...
runSimpleIntegrationTest()
  .then(success => {
    console.log(`简化版集成测试${success ? '通过' : '失败'}`);
