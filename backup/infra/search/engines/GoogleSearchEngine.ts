...
export class GoogleSearchEngine implements SearchEngine {
  ...
  setProxy(proxyServer: string): void {
    ...
    logger.info({ proxy: proxyServer }, '设置代理服务器');
  }
  ...
  private async getAPISuggestions(
    ...
    try {
      logger.info({ keyword }, '获取Google搜索建议');
      ...
      logger.error({ error }, 'API获取搜索建议失败');
      throw new AppError(
  ...
  async getSuggestions(
    ...
    if (this.cookie) {
      try {
        ...
        logger.warn({ error }, 'API模式失败，切换到浏览器模式');
      }
    }
  ...
  async getWebpageContent(url: string, options?: SearchOptions): Promise<string> {
    ...
    try {
      ...
      logger.debug(`成功获取网页内容，内容长度: ${content.length}`);
      return content;
    } catch (error) {
      if (this.page) {
        await this.saveErrorScreenshot(this.page, 'error-google-webpage-content');
      }
      throw new AppError(
        `获取网页内容失败: ${(error as Error).message}`,
        ErrorType.BROWSER,
        error as Error
      );
    }
