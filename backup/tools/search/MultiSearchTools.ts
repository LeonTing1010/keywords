...
export class MultiSearchTools {
  ...
  constructor(config: MultiSearchToolsConfig = {}) {
      ...
      try {
        ...
        switch (engineType.toLowerCase()) {
          ...
          case 'mock':
            ...
            break;
          default:
            logger.warn({ engineType }, `未知的搜索引擎类型: ${engineType}，将使用默认引擎`);
            continue;
        }
        ...
        logger.info({ engineType }, `搜索引擎已注册: ${engineType}`);
      } catch (error) {
        logger.error({ error }, `初始化搜索引擎失败: ${engineType}`);
      }
    }
    ...
    logger.info({ engines: Array.from(this.engines.keys()), defaultEngine: this.defaultEngine }, `MultiSearchTools初始化完成，共${this.engines.size}个引擎`);
  }
  ...
  async initialize(): Promise<void> {
    const initPromises = Array.from(this.engines.entries()).map(async ([name, engine]) => {
      try {
        ...
        logger.info({ name }, `搜索引擎初始化成功: ${name}`);
        return true;
      } catch (error) {
        logger.error({ error }, `搜索引擎初始化失败: ${name}`);
        return false;
      }
    });
  ...
  getEngineSelectionTool() {
    ...
    tool._call = async ({ engine, keyword, reason }) => {
      ...
      logger.info({ reason }, `为关键词 "${keyword}" 选择了 ${engine} 搜索引擎`);
      ...
      try {
        ...
        logger.error({ error: error.message }, `使用 ${engine} 搜索失败`);
        ...
        try {
          ...
          logger.error({ backupError }, '备用引擎也失败了');
        }
  ...
  getSmartSearchTool() {
    ...
    tool._call = async ({ keyword, preference, maxResults = 5 }) => {
      ...
      logger.info({ keyword }, `智能选择了 ${chosenEngine} 搜索引擎`);
      ...
      try {
        ...
        logger.error({ engine: chosenEngine, error: error.message }, '智能搜索失败');
        ...
        if (backupEngine) {
          try {
            ...
            logger.error({ error: backupError }, '备用引擎也失败了');
          }
        }
