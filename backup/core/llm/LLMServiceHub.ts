...
import { SchemaValidatedLLMProvider } from './SchemaValidatedLLMProvider';
...
const saveDebugLog = async (prefix: string, content: string): Promise<string> => {
  try {
    ...
    logger.error({ error }, 'Failed to save debug log');
    return '';
  }
};
...
export class LLMServiceHub {
  ...
  constructor(config: LLMServiceConfig = {}) {
    ...
    if (!this.mockMode) {
      ...
      logger.info({ model: this.model, endpoint: this.apiEndpoint }, 'LLM服务中心初始化完成');
    }

    // 根据环境变量或配置确定实际提供者
    if (this.model.startsWith('qwen') || this.model.includes('qwen')) {
      this.provider = {
        call: async (messages, options) => {
          return await this.callQwenAPI(this.messagesToPrompt(messages), options?.temperature || this.temperature, options || {});
        },
        getName: () => 'QwenAPI'
      };
    } else if (this.model.includes('claude')) {
      this.provider = {
        call: async (messages, options) => {
          return await this.callClaudeAPI(this.messagesToPrompt(messages), options?.temperature || this.temperature, options || {});
        },
        getName: () => 'ClaudeAPI'
      };
    } else {
      ...
      this.provider = {
        call: async (messages, options) => {
          return await this.callOpenAIAPI(this.messagesToPrompt(messages), options?.temperature || this.temperature, options || {});
        },
        getName: () => 'OpenAIAPI'
      };
    }
  ...
  public async analyze(prompt: string, analysisType: string, options: AnalyzeOptions = {}): Promise<any> {
    try {
      ...
      options.analysisType = options.analysisType || analysisType;
      // 如果请求的是JSON格式，默认启用严格模式
      if (options.format === 'json' && options.strictFormat === undefined) {
        options.strictFormat = true;
      }
      
      logger.info({ task: analysisType, model: this.model}, '开始LLM分析');
      
      // 保存提示词用于调试
      if (prompt.length > 200) {
        const logPath = await saveDebugLog(analysisType+'_llm_prompt', prompt);
        if (logPath) {
          logger.debug({ logPath }, '分析提示词已保存到文件');
        }
        // logger.debug({ promptPreview: prompt.substring(0, 200) + '...' }, '分析提示词（摘要）');
      } else {
        // logger.debug({ prompt }, '分析提示词');
      }
      
      // 生成请求ID便于跟踪
      const requestId = analysisType + '_' + crypto.randomBytes(8).toString('hex');
      
      logger.debug({ requestId }, `准备调用LLM API [${requestId}]`);
      ...
      while (retryCount < maxRetries) {
        try {
          ...
          logger.info({ model, source: analysisType, output: JSON.stringify(response).substring(0, 200) + (JSON.stringify(response).length > 200 ? '...' : '') }, 'LLM调用输出');
          
          logger.debug({ requestId }, `LLM API调用成功 [${requestId}]`);

          try {
              // 如果响应过长，保存到调试日志文件
              const logPath = await saveDebugLog(analysisType+'_llm_response', typeof response === 'string' ? response : JSON.stringify(response, null, 2));
              if (logPath) {
                logger.debug({ logPath }, '完整LLM响应已保存到文件');
              }
            // 解析响应格式
  ...
  private async parseResponse(response: any, format: 'json' | 'text' | 'markdown' = 'json'): Promise<any> {
    ...
    logger.debug({ format, responseType: typeof response, responseLength: responseStr.length, previewContent: responseStr.substring(0, 100) + (responseStr.length > 100 ? '...' : '') }, '解析LLM原始响应');
   ...
   * 批量分析文本
   */
  public async analyzeBatch(prompts: string[],options: AnalyzeOptions): Promise<any[]> {
    return Promise.all(prompts.map(prompt => this.analyze(prompt, options.analysisType? 
      options.analysisType: 'batch-request', options)));
  }
  ...
  /**
   * 创建一个支持Schema验证的LLM提供者
   */
  public createSchemaValidatedProvider(provider: LLMProvider): LLMProvider {
    return new SchemaValidatedLLMProvider(provider);
  }
  ...
  public getDefaultProvider(): LLMProvider {
    ...
    return {
      call: async (messages, options) => {
        ...
        // 根据模型选择API
        if (this.model.startsWith('qwen') || this.model.includes('qwen')) {
          return await this.callQwenAPI(prompt, temperature, options || {});
        } else if (this.model.includes('claude')) {
          return await this.callClaudeAPI(prompt, temperature, options || {});
        } else {
          // 默认使用OpenAI
          return await this.callOpenAIAPI(prompt, temperature, options || {});
        }
      },
      getName: () => `${this.model}Provider`
    };
  }
