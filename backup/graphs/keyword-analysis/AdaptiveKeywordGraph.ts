...
export function createAdaptiveWorkflow(
  ...
  const workflowConfig: WorkflowConfig = {
    ...
    steps,
    onComplete: (result) => logger.info({ result }, '自适应工作流完成'),
    onError: (error) => logger.error({ error }, '自适应工作流出错')
  };
  ...
  return new RunnableLambda({
    func: async (state: any, config?: RunnableConfig) => {
      try {
        ...
        logger.info({ currentNode }, `执行节点 ${currentNode}`);
        logger.debug({
        ...
        }, `Node input state for ${currentNode}`);
        ...
        logger.error({
          ...
          stack: error.stack,
          name: error.name
        }, `Error executing node ${agent.constructor.name}`);
