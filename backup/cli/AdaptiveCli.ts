...
import { instrumentAgent } from '../tools/utils/AgentInstrumenter';
...
if (process.env.LLM_BASE_URL) {
  ...
  logger.info({ baseUrl: process.env.LLM_BASE_URL }, `设置OPENAI_BASE_URL为: ${process.env.LLM_BASE_URL}`);
}
...
async function main() {
  try {
    ...
    logger.info({ keyword: argv.keyword }, `开始分析关键词: "${argv.keyword}"`);
    logger.info({ mode: argv.fast ? '快速模式' : '完整模式' }, `模式: ${argv.fast ? '快速模式' : '完整模式'}`);
    
    // 初始化Agent跟踪系统
    if (argv['track-agents']) {
      // 设置环境变量以便其他模块使用
      process.env.AGENT_TRACKING_ENABLED = "true";
      process.env.AGENT_TRACKING_DIR = argv['track-output'];
      
      // 初始化跟踪管理器
      agentTrackingManager.initialize({
        enabled: true,
        outputDirectory: argv['track-output'],
        includeAllMethods: true
      });
      
      logger.info(
        { outputDir: argv['track-output'] }, 
        "已启用Agent跟踪功能"
      );
    } else {
    ...
    const marketNeedExplorerAgent = new MarketNeedExplorerAgent({ 
      ...
      trackExecution: argv['track-agents'] // 启用跟踪
    });
    ...
    const workflow = createAdaptiveWorkflow(
      {
        marketNeedExplorerAgent: marketNeedExplorerAgent as any,
        userJourneySimulatorAgent: userJourneySimulatorAgent as any,
        solutionEvaluatorAgent: solutionEvaluatorAgent as any,
        opportunityStrategistAgent: opportunityStrategistAgent as any
      },
      {
        fastMode: argv.fast,
    ...
    logger.info({ duration: duration.toFixed(2) }, `分析完成! 用时: ${duration.toFixed(2)}秒`);
